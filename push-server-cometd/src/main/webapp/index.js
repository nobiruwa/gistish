  ...
  window.addEventListener('DOMContentLoaded', function() {
    ...
    function Chat(state) {
      ...
      this.join = function(username) {
        ...
        var cometdURL = window.location.protocol + '//' + window.location.host + '/push-server-cometd/cometd'; // location.protocol + "//" + location.host + state.config.contextPath + "/cometd";
        ...
        _cometd.unregisterTransport('callback-polling');
        _cometd.handshake();
      ...
      _cometd.addListener('/meta/handshake', function(message) {
        if (message.successful)
        {
            document.getElementById('status').innerHTML += '<div>CometD handshake successful</div>';
            _cometd.subscribe('/stock/meigara', function(message)
            {
                ...
                var $symbolDiv = $('#' + id);
                if ($symbolDiv.length === 0)
                {
                  $symbolDiv = $('<div id="' + id + '"></div>');
                  $symbolDiv.appendTo($('#stocks'));
                }
              $symbolDiv.append($('<span>' + symbol + ': ' + value + '</span>'));
            });
        }
        else
        {
